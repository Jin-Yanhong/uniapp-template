            ...
            <view class="details flexLayout">
                <view class="top flexLayout">
                    <view class="left flexLayout">
                        ...
                        <view v-if="isLogin">
                            ...
                            <view v-else class="userName">
                                <text class="name textEllipsis">{{ user.name || user.nickname || defaultInfo.nickName }}</text>
                                <view class="isNotMember">非會員</view>
                            </view>
                            <view style="font-size: 24rpx; color: #faeee0" v-if="user.isMember && user.school">{{ user.school }}</view>
                        </view>
                        <view v-else @click="navTo('/pages/login/index', false, navTypeList.switchTab)">
                            <view class="userName">
                                <text class="name">未登錄</text>
                    ...
                    <view class="right" v-if="!user.isMember">
                        ...
                        <view class="rightBtn isNotMember" @click="showToast('Taped!')">XXXX</view>
                    </view>
                </view>
                <view class="bottom flexLayout">
                    ...
                    <view class="right">
                        <view class="settings" @click="showToast('Taped!')">
...
import { confirmModal, getStorageItem, isLogin, isVip, removeStorageItem, showToast } from '../../utils';
export default {
    data() {
        return {
            ...
            isLogin: false,
            msgCount: 4,
            navList: [{ icon: 'image/icon/mine-message.png', isMerchantFeature: false, text: '消息通知' }],
        };
    },
    onShow() {
        // this.handleShow();
